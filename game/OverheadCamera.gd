...
func _process(delta: float) -> void:
	var distance_coef: float = 1.0 / maxf(global_position.distance_to(follow.global_position) * 0.2, 1.0)
	var pos_lerp_rate: float = clamp(POS_LERP_SPEED * distance_coef * delta, 0.0, 1.0)
